...
class CephStorageClient(object):
    ...
    def upload_file_from_path(self, file_path, container=None):
        ...
        # Assume filename format: E123N4567_LAZ_DRM_U1_1.laz
        ...
        self.log.info("Uploading  file {0}  \
                    [size:{1} | type:{2}]...".format(obj_meta_dict['name'],
                                                     obj_meta_dict['bytes'],
    ...
    def download_file_to_path(self, object_name, destination_path, container=None):
        obj_tuple = self.connection.get_object(
            self.active_container_name, object_name)
        ...
        self.log.info("Finished downloading to [{0}]. \
            Wrote [{1}] bytes...".format(file_path,
    ...
    def log_wrapper(self):
        ...
        """
        # Initialize logging
        logging.basicConfig(filename=self.get_cwd() +
                            'logs/ceph_storage.log', level=logging.DEBUG)
        self.LOGGING = True
