...
# parameters
muni_boundary = r"D:\update_muni_index\FHM_Monitoring.gdb\PSA_Muni_Boundary"
fhm_coverage = r"D:\update_muni_index\FHM_Monitoring.gdb\fhm_coverage"
builtup = r"D:\update_muni_index\FHM_Monitoring.gdb\builtup_all"
muni_index = r"D:\update_muni_index\FHM_Monitoring.gdb\PSA_Muni_Index"
...
# variables
temp_dir = r"D:\update_muni_index\FHM_Monitoring.gdb"
fhm_muni_intersect = os.path.join(temp_dir, "fhm_muni_intersect")
fhm_muni_dissolve = os.path.join(temp_dir ,"fhm_muni_dissolve")
fhm_muni_builtup = os.path.join(temp_dir, "fhm_muni_builtup")
fhm_muni_point = os.path.join(temp_dir, "fhm_muni_point")
fhm_muni_sj = os.path.join(temp_dir, "fhm_muni_sj")

try:
	# delete intermediate data
	arcpy.Delete_management(fhm_muni_intersect)
	arcpy.Delete_management(fhm_muni_dissolve)
	arcpy.Delete_management(fhm_muni_builtup)
	arcpy.Delete_management(fhm_muni_point)
	arcpy.Delete_management(fhm_muni_sj)
except Exception, e:
	print e
	sys.exit()
...
arcpy.JoinField_management(muni_index, "MUN_CODE", fhm_muni_sj, "MUN_CODE", "RBFP_name;Processor;SUC")
...
arcpy.CalculateField_management(muni_index, "RBFP", "sortValues(!RBFP_name!)", "PYTHON_9.3", codeblock_sort)
...
arcpy.MakeFeatureLayer_management(fhm_coverage, "fhm_coverage_layer", "proj_res = 30m")
arcpy.SelectLayerByLocation_management("muni_index_layer", "INTERSECT", "fhm_coverage", "", "NEW_SELECTION")

def checkResolution(fhm_res, index_res):
	proj_res = ""
	if fhm_res == "30m":
		if index_res == "10m":
			proj_res = "30m_10m"
